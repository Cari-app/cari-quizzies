...
import { useDeviceContext } from '../ComponentEditor';
...
export function ImageButtonComponentTab({ 
  ...
  // Get device context from ComponentEditor
  const selectedDevice = useDeviceContext();

  const items: ImageButtonItem[] = config.imageButtonItems || [];

  // Get the correct config key based on device
  const getLayoutValue = () => {
    if (selectedDevice === 'mobile') {
      return config.imageButtonLayoutMobile || config.imageButtonLayout || 'list';
    }
    return config.imageButtonLayout || 'list';
  };

  // Update the correct config key based on device
  const setLayoutValue = (value: string) => {
    if (selectedDevice === 'mobile') {
      updateConfig({ imageButtonLayoutMobile: value });
    } else if (selectedDevice === 'desktop') {
      // When setting desktop, also clear mobile if it was the same
      updateConfig({ imageButtonLayout: value });
    } else {
      // "all" - set base and clear mobile override
      updateConfig({ imageButtonLayout: value, imageButtonLayoutMobile: undefined });
    }
  };
