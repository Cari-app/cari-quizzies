...
export function AppearanceEditor({ config, onUpdate, componentType, themeColor }: AppearanceEditorProps) {
  const showLabelStyle = ['input', 'email', 'phone', 'number', 'date', 'textarea', 'height', 'weight'].includes(componentType);
  const showTextAlign = ['text', 'button', 'input', 'email', 'phone', 'number', 'date', 'textarea'].includes(componentType);
  const showFontSize = ['text', 'button'].includes(componentType);
